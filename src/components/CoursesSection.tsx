...
export default function CoursesSection() {
  ...
  return (
    <section ref={sectionRef} id="courses" className="bg-white py-24 px-[60px] font-nb max-md:px-6">
      <div className="max-w-[1240px] mx-auto">
        ...
        <div className="mb-16">
          ...
          <div className="flex items-end justify-between gap-8 max-md:flex-col max-md:items-start">
            <h2 className="text-5xl font-normal leading-[1.05] tracking-[-0.03em] text-black max-w-[560px] max-md:text-[36px]">
            ...
            <p ref={descRef} className="text-[15px] text-black/45 leading-[1.7] max-w-[420px] tracking-[0.003em] pb-1">
        ...
        {/* ── Course grid ── */}
        <div ref={gridRef} className="grid grid-cols-3 max-md:grid-cols-1 max-lg:grid-cols-2">
          {COURSES.map(({ n, icon, title, desc, tags, cta }, i) => {
            const isLastCol  = (i + 1) % 3 === 0
            const isLastItem = i === COURSES.length - 1
            return (
              <div
                key={n}
                className={[
                  'group flex flex-col gap-5 p-8 border-black/[0.07]',
                  'border-b',
                  isLastCol ? '' : 'border-r',
                  isLastItem && COURSES.length % 3 !== 0 ? 'border-r-0' : '',
                ].join(' ')}
              >
                {/* Top row: number */}
                ...
                {/* Title */}
                <h3 className="text-[19px] font-normal tracking-[-0.01em] text-black leading-snug">
                  {title}
                </h3>

                {/* Description */}
                <p className="text-[13.5px] text-black/45 leading-[1.68] tracking-[0.003em] flex-1">
                  {desc}
                </p>
