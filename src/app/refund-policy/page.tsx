import PolicyLayout, { type Policy } from '@/components/PolicyLayout'

export const metadata = { title: 'Refund & Cancellation Policy · Delta Trading Academy' }

const POLICY: Policy = {
  "title": "Refund & Cancellation Policy",
  "lastUpdated": "24 July 2026",
  "intro": "This Refund & Cancellation Policy explains how cancellations, seat transfers, and refunds work for the Forex Trading Bootcamp seminar organised by Delta International Institute, operating as Delta Trading Academy (\"Delta\", \"we\", \"us\", \"our\"). It applies to every ticket purchased through our website for the one-day, in-person Forex Trading Bootcamp held at The Oberoi, Bengaluru. Please read it carefully before completing your payment, as your registration and payment confirm that you accept these terms.",
  "sections": [
    {
      "heading": "Overview",
      "paragraphs": [
        "The Forex Trading Bootcamp is a paid, one-day, in-person trading seminar priced at INR 299 per seat. It includes live trading sessions, a premium buffet lunch, and a graduation ceremony, and seating is strictly limited to between 60 and 72 attendees per event.",
        "Because seating is capacity-limited and we commit to venue, catering, and staffing costs in advance, we operate a clear, tiered cancellation and refund structure. This policy is designed to be fair to both attendees and to Delta, and to comply with applicable Indian consumer-protection norms.",
        "All payments for the seminar are collected online through Razorpay, which supports UPI, credit and debit cards, and netbanking. We do not store or have access to your card or UPI credentials; these are handled securely by Razorpay. Any approved refund is therefore returned through Razorpay to the original payment method used at the time of purchase."
      ]
    },
    {
      "heading": "Your Ticket and What It Covers",
      "paragraphs": [
        "Each INR 299 ticket entitles one named person to attend a single scheduled Forex Trading Bootcamp event on the date selected at the time of registration."
      ],
      "bullets": [
        "Ticket price: INR 299 per seat (inclusive of applicable taxes as shown at checkout).",
        "The ticket covers admission to the live trading sessions, the premium buffet lunch, and the graduation ceremony for the selected event date only.",
        "Tickets are issued against the full name, email address, and mobile number provided during registration.",
        "Tickets are valid solely for the specific event date and venue for which they were purchased and cannot be applied to a different date except as set out in this policy."
      ]
    },
    {
      "heading": "Cancellation by You (Refund Tiers)",
      "paragraphs": [
        "If you wish to cancel your registration and request a refund, the amount refunded depends on how far in advance of the event start date and time you submit your cancellation request to us. The following tiers apply to the INR 299 ticket price:"
      ],
      "bullets": [
        "Cancellation 7 or more days before the event: full refund of 100% of the ticket price (INR 299).",
        "Cancellation less than 7 days but at least 72 hours (3 days) before the event: partial refund of 50% of the ticket price.",
        "Cancellation within 72 hours (3 days) of the event start time: no refund is available, as the seat and associated venue and catering costs are already committed.",
        "The applicable tier is determined by the date and time we receive your written cancellation request by email, not the date you decided to cancel."
      ]
    },
    {
      "heading": "Transfer Your Seat Instead of Cancelling",
      "paragraphs": [
        "If you are unable to attend, you may transfer your seat to another person instead of cancelling, at no additional charge. This is often the best option when a refund would be reduced or unavailable under the tiers above.",
        "To arrange a transfer, email us at info@deltainstitutions.com at least 48 hours before the event with your order or payment ID and the full name, email address, and mobile number of the replacement attendee. Once we confirm the transfer in writing, the new attendee will be admitted in your place, and the original ticket will be treated as fulfilled with no further refund due."
      ],
      "bullets": [
        "Seat transfers are free of charge and may be requested up to 48 hours before the event.",
        "Only one transfer is permitted per ticket.",
        "The replacement attendee is bound by the same terms and this policy."
      ]
    },
    {
      "heading": "No-Show Policy",
      "paragraphs": [
        "If you do not attend the seminar on the scheduled date and did not cancel or transfer your seat in advance in accordance with this policy, your registration is treated as a no-show. No-shows are not eligible for any refund, credit, or transfer to a future event, as your seat was reserved and the associated costs were incurred on your behalf."
      ]
    },
    {
      "heading": "Cancellation or Rescheduling by Us",
      "paragraphs": [
        "In rare circumstances, we may need to cancel or reschedule an event, for example due to venue issues, insufficient registrations, or events beyond our reasonable control. If this happens, we will notify you promptly using the email address and mobile number provided at registration.",
        "If we cancel or reschedule an event, you are fully protected and may choose one of the options below."
      ],
      "bullets": [
        "A full refund of 100% of your ticket price (INR 299), regardless of how close to the event the change occurs; or",
        "A free transfer of your registration to the new or a future scheduled date of the Forex Trading Bootcamp at no additional cost.",
        "If we do not hear from you within a reasonable period after a cancellation by us, we will process a full refund to your original payment method by default."
      ]
    },
    {
      "heading": "How to Request a Refund or Cancellation",
      "paragraphs": [
        "All cancellation, refund, and transfer requests must be made in writing by email so that we have an accurate record of the request time, which determines your refund tier.",
        "Please email info@deltainstitutions.com from the email address used at registration and include the details below. We will acknowledge your request and confirm the outcome in writing."
      ],
      "bullets": [
        "Your full name as entered during registration.",
        "Your order ID or Razorpay payment ID from your booking confirmation.",
        "The registered mobile number and email address.",
        "The event date and, where relevant, whether you prefer a refund or a seat transfer.",
        "You may also call us at +91 91872 36407 for assistance, but the formal request must be confirmed by email."
      ]
    },
    {
      "heading": "Refund Method and Timeline",
      "paragraphs": [
        "All approved refunds are processed through Razorpay to the original payment method used for the booking (the same UPI ID, card, or bank account). We are unable to redirect a refund to a different account or payment instrument.",
        "Once your refund is approved, we will initiate it through Razorpay within 2 business days. The refunded amount is typically credited to your original payment method within 5 to 7 business days of initiation, though the exact time to reflect in your account depends on your bank or payment provider. You will receive a confirmation once the refund has been initiated."
      ],
      "bullets": [
        "Refund destination: the original payment method used at checkout, via Razorpay.",
        "Refund initiation: within 2 business days of approval.",
        "Time to credit: normally 5 to 7 business days after initiation, subject to your bank or card issuer.",
        "Refund amount: as per the applicable tier in Sections 3 and 6; any statutory taxes are refunded proportionately with the refunded ticket amount."
      ]
    },
    {
      "heading": "General Conditions",
      "bullets": [
        "This policy applies only to tickets purchased directly through our official website at https://deltainstitutions.com/seminar (also served at https://deltatradinghub.com/seminar).",
        "Refund tiers are calculated from the scheduled event start date and time as shown on your booking confirmation.",
        "We reserve the right to decline duplicate, fraudulent, or bad-faith refund requests, and to verify the identity of the person making a request.",
        "Nothing in this policy limits any rights you may have under applicable Indian consumer-protection law.",
        "We may update this policy from time to time; the version in effect at the time of your purchase governs your booking."
      ]
    },
    {
      "heading": "Educational Nature and Risk Disclaimer",
      "paragraphs": [
        "The Forex Trading Bootcamp is an educational seminar. All content, live sessions, and materials are provided for educational purposes only and do not constitute investment, financial, legal, or tax advice, nor a recommendation to buy or sell any security or financial instrument. Trading in financial markets carries substantial risk, and past performance is not indicative of future results. You are solely responsible for your own trading and investment decisions. This disclaimer does not affect your refund rights set out above."
      ]
    },
    {
      "heading": "Contact Us",
      "paragraphs": [
        "For any question about cancellations, transfers, or refunds, please contact us using the details below. We aim to respond to all refund-related queries promptly during business hours."
      ],
      "bullets": [
        "Business name: Delta International Institute (operating as Delta Trading Academy).",
        "Email: info@deltainstitutions.com",
        "Phone: +91 91872 36407",
        "Registered address: #412A, 4th Floor, Prestige Towers, Residency Road, Bengaluru, Karnataka 560025, India.",
        "GST No: 29AAJCE53644R1ZP",
        "Governing law and jurisdiction: this policy is governed by the laws of India, and any disputes are subject to the exclusive jurisdiction of the courts of Bengaluru, Karnataka."
      ]
    }
  ]
}

export default function RefundPolicyPage() {
  return <PolicyLayout policy={POLICY} />
}
